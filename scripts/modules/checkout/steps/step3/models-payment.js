    ...
    function($, _, Hypr, Backbone, api, CustomerModels, AddressModels, PaymentMethods, HyprLiveContext, CheckoutStep, ShippingDestinations) {
        ...
        var BillingInfo = CheckoutStep.extend({
            ...
            relations: {
                ...
                GSIPaypal: PaymentMethods.Paypal
            },
            ...
            validateBillingAddress: function() {
                ...
                if (!isValid && $('[data-mz-value="contactId"]:visible').length)
                    return Hypr.getLabel('genericRequired');
            },
            validateBillingEmail: function(value, property, model) {
                var email = document.getElementById('billing-email').value;
                // Matches a valid email address (e.g. mail@example.com) 
                var isValid = Backbone.Validation.patterns.email.test(email);
                if (!isValid)
            ...
            getDigitalCredit: function() {
                ...
                me.isLoading(true);
                return me.retrieveDigitalCredit(customer, creditCode, me)
                    .then(function() {
                        //console.log('applied');
                    }, function(err) {
                        if (err.message.indexOf('Item not found:') !== -1) {
                            window.payment.trigger('error', { message: Hypr.getLabel('promoCodeError', creditCode) });
                        }
                    }).ensure(function() {
                        me.isLoading(false);
                        return me;
            ...
            isPaypalEnabled: function() {
                var me = this,
                    currentPaypal = me.get('GSIPaypal');
                currentPaypal.initialize();
                var sitecontext = require.mozuData('sitecontext');
                if (sitecontext.checkoutSettings && sitecontext.checkoutSettings.externalPaymentWorkflowSettings) {
                    return _.where(sitecontext.checkoutSettings.externalPaymentWorkflowSettings, { "name": "GSIPaypal" }).length > 0 ? true : false;
                } else {
                    return false;
                }
            },
            ...
            setPaypalInfo: function() {
                var me = this,
                    order = me.getOrder(),
                    isPaypalEnabled = me.isPaypalEnabled(),
                    currentPaypal = me.get('GSIPaypal');
                if (!currentPaypal) {
                    return;
                }
            ...
            selectPaymentType: function(me, newPaymentType) {
                ...
                if (me.get('GSIPaypal')) {
                    me.get('GSIPaypal').selected = newPaymentType === 'GSIPaypal';
                }
            ...
            setDefaultPaymentType: function(me) {
                ...
                } else if (me.isPaypalEnabled()) {
                    me.set('paymentType', 'GSIPaypal');
                    me.selectPaymentType(me, 'GSIPaypal');
            ...
            calculateStepStatus: function() {
                ...
                if (thereAreActivePayments && (balanceNotPositive || (this.get('paymentType') === 'GSIPaypal' && window.location.href.indexOf('GSIPaypal=complete') !== -1))) return this.stepStatus('complete');
            ...
            toJSON: function(options) {
                var j = CheckoutStep.prototype.toJSON.apply(this, arguments),
                    loggedInEmail;
