...
var CheckoutPage = Backbone.MozuModel.extend({
            ...
            saveCustomerContacts: function() {
                ...
                // }
                var billingContact = "";
                if(this.get('billingInfo').get('billingContact').get('address').get('address1')){
                    billingContact = this.get('billingInfo').get('billingContact').toJSON();
                    delete billingContact.email;
                    billingContact.types =  [{
                            "name": "Billing",
                            "isPrimary": true
                        }];
                    var existingBillingContactIndex = existingContacts.length > 0 ? self.getContactIndex(existingContacts, billingContact) : -1;
                    var updatedContactIndex = updatedContacts.length > 0 ? self.getContactIndex(updatedContacts, billingContact) : -1;
